...
mydata <- right_join(data, group) %>% 
  mutate(
    Group = factor(
      Group,
      levels = c("Control", "L.fermentum", "L.animalis", "L.salvarius", "L.reuteri", "L.reuteriAPS", "APS")
    )
  ) %>% 
  arrange(Group)
...
ggsave(filename = "chao_sd51.pdf", plot = p_sd, height = 6, width = 6)
...
ggsave(filename = "shannon_sd51.pdf", plot = p_sd2, height = 6, width = 6)

pp <- p_sd + p_sd2
ggsave(filename = "combined_sd51.pdf", plot = pp, height = 6, width = 8)
